...
exports.specialForms = {
    'def': (args, env, interpret, nsManager) => {
        if (args.length !== 2) {
    ...
    'defn': (args, env, interpret, nsManager) => {
        if (args.length < 3) {
    ...
    'fn': (args, env, interpret, nsManager) => {
        if (args.length !== 2) {
    ...
    'let': (args, env, interpret, nsManager) => {
        if (args.length < 2) {
        ...
        const bindings = bindingList.value;
        if (bindings.length % 2 !== 0) {
            throw new Error('let binding list must have an even number of elements');
        }
        const letEnv = new Context_1.Environment(env);
    ...
    'loop': (args, env, interpret, nsManager) => {
        if (args.length !== 2) {
        ...
        const loopEnv = new Context_1.Environment(env);
        const paramNames = [];
        const initialValues = [];
        for (let i = 0; i < bindings.length; i += 2) {
            const name = bindings[i];
    ...
    'quote': (args, env, interpret, nsManager) => {
        if (args.length !== 1) {
            throw new Error('quote requires exactly 1 argument');
        }
        return args[0];
    },
    'do': (args, env, interpret, nsManager) => {
        let result = { type: 'nil', value: null };
    ...
    'ns': (args, env, interpret, nsManager) => {
        if (!nsManager) {
...
function processImport(importClauses, nsManager) {
...
function processRequire(requireClauses, nsManager) {
